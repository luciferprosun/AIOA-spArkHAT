...
# Source: cloudops/provider.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
class SyntheticResourceReader:
    ...
    def get_resource(self, query: ResourceQuery) -> CloudResource:
        ...
        if "get_resource" in self._fail_operations:
            raise CloudAdapterUnavailableError(
                "mock cloud adapter failure was injected"
            )
        ...
        if resource is None:
            raise CloudResourceNotFoundError(
                "resource was not found in the selected inventory"
            )
        return resource
    ...
    def describe_instances(self, *, InstanceIds: list[str]) -> Mapping[str, Any]:
        ...
        if "describe_instances" in self._fail_operations:
            raise CloudAdapterUnavailableError(
                "mock DescribeInstances failure was injected"
            )
        if len(InstanceIds) != 1:
            raise CloudAdapterUnavailableError(
                "mock adapter accepts exactly one instance ID"
            )
        ...
        if not isinstance(resource, Ec2Resource):
            raise CloudAdapterUnavailableError(
                "mock instance fixture normalized incorrectly"
            )
        return {
            "Reservations": [
                {
