...
# Source: nz/contracts.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
class BudgetCounters(NonZeroContract):
    ...
    def validate_consumption(self) -> Self:
        ...
        if self.elapsed_milliseconds_used > self.max_elapsed_seconds * 1_000:
            raise ValueError(
                "elapsed_milliseconds_used must not exceed the time budget"
            )
        return self
...
class Ec2Resource(NonZeroContract):
    ...
    resource_type: Literal[CloudResourceType.EC2_INSTANCE] = (
...
class SecurityGroupResource(NonZeroContract):
    ...
    resource_type: Literal[CloudResourceType.SECURITY_GROUP] = (
...
class ResourceEvidence(NonZeroContract):
    ...
    def validate_binding(self) -> Self:
        ...
        if CloudFinding.CLEAN in self.findings and self.findings != (
            CloudFinding.CLEAN,
        ):
            raise ValueError("CLEAN cannot coexist with a remediation finding")
        if (
            tuple(sorted(set(self.findings), key=lambda item: item.value))
            != self.findings
        ):
    ...
    def create(
        ...
        *,
        ...
        return cls(
            **values, evidence_hash=_canonical_digest(provisional.evidence_payload())
        )
...
class RemediationProposal(NonZeroContract):
    ...
    def create(
        ...
        *,
        ...
        return cls(
            **values, proposal_hash=_canonical_digest(provisional.action_payload())
        )
...
class ActionProposal(NonZeroContract):
    ...
    def validate_mutation_proposal(self) -> Self:
        if self.action is not Capability.STOP_SANDBOX_INSTANCE:
            raise ValueError(
                "only the canonical sandbox stop may form a mutation proposal"
            )
        require_capability_authority(self.action, self.authority)
        if (
            self.expected_precondition.instance_state
            is not ObservedInstanceState.RUNNING
        ):
    ...
    def validate_outcome(self) -> Self:
        if self.outcome is ActionOutcome.SUCCEEDED:
            if (
                ...
                raise ValueError(
                    "successful action result requires evidence and forbids failure"
                )
        elif self.failure is None:
            raise ValueError(
                "non-success action result requires explicit failure detail"
            )
        return self
...
class ExecutionAcknowledgement(NonZeroContract):
    ...
    target: ActionTarget
    previous_state: Literal[ObservedInstanceState.RUNNING] = (
        ObservedInstanceState.RUNNING
    )
    current_state: Literal[
        ObservedInstanceState.STOPPING, ObservedInstanceState.STOPPED
    ]
...
class VerificationEvidence(NonZeroContract):
    ...
    trace_id: Uuid7Identifier
    correlation_id: Uuid7Identifier
    disposition: Literal[VerificationDisposition.VERIFIED] = (
        VerificationDisposition.VERIFIED
    )
    action: Literal[Capability.STOP_SANDBOX_INSTANCE] = Capability.STOP_SANDBOX_INSTANCE
    target: ActionTarget
    observed_state: Literal[ObservedInstanceState.STOPPED] = (
        ObservedInstanceState.STOPPED
    )
    verified_at: datetime
    proof_origin: VerificationProofOrigin = (
        VerificationProofOrigin.EXECUTION_ACKNOWLEDGEMENT
    )
    ...
    def validate_evidence_hash(self) -> Self:
        ...
        if self.evidence_hash != hashlib.sha256(canonical).hexdigest():
            raise ValueError(
                "verification evidence_hash does not match canonical evidence"
            )
        return self
    ...
    def evidence_payload(self) -> dict[str, object]:
        ...
        if self.proof_origin is VerificationProofOrigin.EXECUTION_ACKNOWLEDGEMENT:
            ...
            payload["execution_acknowledgement_hash"] = (
                self.execution_acknowledgement_hash
            )
        else:
    ...
    def create(
        ...
        *,
        ...
        if (
            ...
            raise ValueError(
                "verification evidence prerequisites do not share one identity"
            )
        values: dict[str, object] = {
    ...
    def validate_status_payload(self) -> Self:
        if self.status is IdempotencyStatus.REGISTERED:
            if self.action_result is not None or self.completed_at is not None:
                raise ValueError(
                    "registered idempotency record cannot contain a result"
                )
        elif self.action_result is None or self.completed_at is None:
            raise ValueError(
                "resolved idempotency record requires result and completed_at"
            )
    ...
    def create(
        ...
        *,
        ...
        if proposal.authority_class is not AuthorityGate.PLAN_AND_CONFIRM:
            raise ValueError(
                "only PLAN_AND_CONFIRM proposals may request local approval"
            )
        ...
        return cls(
            **values, request_hash=_canonical_digest(provisional.binding_payload())
        )
...
class LocalApprovalDecisionRecord(NonZeroContract):
    ...
    def create(
        ...
        *,
        ...
        return cls(
            **values, decision_hash=_canonical_digest(provisional.binding_payload())
        )
...
class LocalExecutionIntent(NonZeroContract):
    ...
    def validate_intent_binding(self) -> Self:
        if self.operation_type is RemediationOperation.APPLY_REQUIRED_TAGS:
            raise ValueError(
                "NEVER_AUTONOMOUS tag proposals cannot form execution intent"
            )
    ...
    def create(
        ...
        *,
        ...
        return cls(
            **values, intent_hash=_canonical_digest(provisional.binding_payload())
        )
...
class LocalExecutionReceipt(NonZeroContract):
    ...
    def validate_receipt_binding(self) -> Self:
        ...
        if self.operation_type is RemediationOperation.RELEASE_ELASTIC_IP:
            ...
            if (
                self.before_resource.association_id is not None
                or self.after_resource is not None
            ):
                raise ValueError(
                    "release receipt must prove an unattached address became absent"
                )
        elif self.operation_type is RemediationOperation.REVOKE_PUBLIC_INGRESS:
            if not isinstance(
                self.before_resource, SecurityGroupResource
            ) or not isinstance(self.after_resource, SecurityGroupResource):
                raise ValueError(
                    "ingress receipt requires Security Group before/after state"
                )
            if (
                ...
                or self.after_resource.outbound_rules
                != self.before_resource.outbound_rules
                or self.after_resource.tags != self.before_resource.tags
                or len(self.after_resource.inbound_rules)
                >= len(self.before_resource.inbound_rules)
            ):
                raise ValueError(
                    "ingress receipt does not prove a bounded rule removal"
                )
    ...
    def receipt_payload(self) -> dict[str, object]:
        return {
            "after_resource": (
                None
                if self.after_resource is None
                else self.after_resource.model_dump(mode="json")
            ),
    ...
    def validate_verification_binding(self) -> Self:
        if self.operation_type is RemediationOperation.RELEASE_ELASTIC_IP:
            ...
            if self.observed_absent or not isinstance(
                ...
                raise ValueError(
                    "ingress verification requires Security Group read-back"
                )
            if (
...
class Checkpoint(NonZeroContract):
    ...
    tool_result_hashes: dict[ShortIdentifier, Sha256Digest] = Field(
    ...
    def validate_last_safe_state(self) -> Self:
        if self.last_safe_state in {WorkflowState.EXECUTING, WorkflowState.VERIFYING}:
            raise ValueError(
                "checkpoint cannot label an active side-effect state as safe"
            )
        ...
        if self.remediation_proposal is not None and (
            ...
            or self.remediation_proposal.evidence_hash
            != self.resource_evidence.evidence_hash
        ):
