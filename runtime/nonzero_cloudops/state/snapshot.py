...
# Source: persistence/memory.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
from .idempotency import derive_action_fingerprint, derive_idempotency_key
...
class DomainSnapshot:
    """Validated transaction image, always persisted by DomainStateRepository.

    This is not exposed as a durability fallback or separate application.
    """
    ...
    def create_run(self, run: Run) -> Run:
        if run.version != 1 or run.state is not WorkflowState.RECEIVED:
            raise StorageConflictError(
                "new durable run must start at RECEIVED version 1"
            )
    ...
    def transition_run(
        ...
        *,
        ...
        if next_state in {WorkflowState.APPROVED, WorkflowState.DENIED_BY_HUMAN}:
            ...
            local_approval = (
                checkpoint.local_approval if checkpoint is not None else None
            )
        ...
        if next_state is WorkflowState.SUCCESS_WITH_EVIDENCE:
            if verification_proposal_id is None:
                raise StorageConflictError(
                    "SUCCESS_WITH_EVIDENCE requires durable verification"
                )
            proposal = self._proposals.get(verification_proposal_id)
            evidence = self._verification_evidence.get(
                (run_id, verification_proposal_id)
            )
            idempotency = (
    ...
    def update_run_budget(
        ...
        *,
        ...
        if (
            ...
            or budget.elapsed_milliseconds_used
            < current.budget.elapsed_milliseconds_used
    ...
    def create_approval(self, approval: Approval) -> Approval:
        ...
        if proposal is None or proposal.state is not ProposalState.AWAITING_APPROVAL:
            raise StorageConflictError(
                "human decision requires an awaiting durable proposal"
            )
    ...
    def register_idempotency(self, record: IdempotencyRecord) -> IdempotencyRecord:
        ...
        if (
            ...
            raise StorageConflictError(
                "idempotency registration lacks a matching proposal"
            )
        ...
        if approval is None or approval.decision is not ApprovalDecision.APPROVED:
            raise StorageConflictError(
                "idempotency registration requires human approval"
            )
        if run is None or run.state is not WorkflowState.APPROVED:
            raise StorageConflictError(
                "idempotency registration requires an approved run"
            )
    ...
    def list_audit_events(
        self, run_id: UUID, *, limit: int = 128
    ) -> tuple[AuditEvent, ...]:
        ...
        events = (
            event
            for (event_run_id, _), event in self._audit_events.items()
            if event_run_id == run_id
        )
        return tuple(
            sorted(events, key=lambda event: (event.timestamp, str(event.event_id)))[
                :limit
            ]
        )
    ...
    def create_verification_evidence(
        ...
        if (
            ...
            raise StorageConflictError(
                "verification evidence does not match the proposal"
            )
    ...
    def export_snapshot(self) -> dict[str, object]:
        ...
        return {
            ...
            "audit_events": [
                ...
                for key in sorted(
                    self._audit_events, key=lambda item: (str(item[0]), str(item[1]))
                )
            ],
    ...
    @classmethod
    def from_snapshot(cls, payload: object) -> "DomainSnapshot":
        ...
        if payload.get("format_version") != 1:
            raise StorageDependencyError(
                "local durable snapshot version is unsupported"
            )
        ...
        def records(name: str, model: Any) -> list[Any]:
            ...
            if not isinstance(values, list) or len(values) > _MAX_SNAPSHOT_RECORDS:
                raise StorageDependencyError(
                    f"local durable snapshot {name} is invalid"
                )
            try:
        ...
        repository._idempotency = {
            record.idempotency_key: record for record in idempotency
        }
        ...
        repository._verification_evidence = {
            ...
            for record in verification_evidence
        }
        counts = (
        ...
        if any(source != restored for source, restored in counts):
            raise StorageDependencyError(
                "local durable snapshot contains duplicate identities"
            )
        ...
        if any(
            approval.proposal_id not in repository._proposals for approval in approvals
        ):
        ...
        if any(count > MAX_AUDIT_EVENTS_PER_RUN for count in event_counts.values()):
            raise StorageDependencyError(
                "local durable snapshot exceeds audit capacity"
            )
        return repository
