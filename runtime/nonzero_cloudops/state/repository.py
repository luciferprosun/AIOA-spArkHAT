...
# Source: persistence/local.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
from ..models.errors import StorageDependencyError
from .files import (
    StateIntegrityError,
...
class DomainStateRepository:
    ...
    def _lock(self, *, exclusive: bool) -> object:
        with self._thread_lock:
            try:
                ...
                raise StorageDependencyError(
                    "local state lock is unavailable"
                ) from error

    def _load(self) -> DomainSnapshot:
        repository, _ = self._load_with_digest()
        return repository

    def _load_with_digest(
        self,
    ) -> tuple[DomainSnapshot, str | None]:
        if not self._path.exists():
            return DomainSnapshot(), None
        try:
            ...
            repository = DomainSnapshot.from_snapshot(payload)
        except StorageDependencyError:
            raise
        except (OSError, StateIntegrityError) as error:
            raise StorageDependencyError(
                "local durable state is corrupt or unreadable"
            ) from error
        return repository, digest

    def _write(self, repository: DomainSnapshot) -> None:
        try:
    ...
    def _read(
        self,
        operation: Callable[[DomainSnapshot], Result],
    ) -> Result:
    ...
    def run_count(self) -> int:
        return self._read(lambda repository: repository.run_count())
    ...
    def get_idempotency(self, idempotency_key: str) -> IdempotencyRecord | None:
        return self._read(
            lambda repository: repository.get_idempotency(idempotency_key)
        )
    ...
    def get_audit_event(self, run_id: UUID, event_id: UUID) -> AuditEvent | None:
        return self._read(
            lambda repository: repository.get_audit_event(run_id, event_id)
        )
    ...
    def read_run_snapshot(self, run_id: UUID, *, limit: int = 128) -> RunSnapshot:
