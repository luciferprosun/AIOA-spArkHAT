...
# Source: agent/local_hitl.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
from .adapters.base import ProtectedExecutor
from .adapters.portable import (
    PortableConflictError,
    PortableMutationError,
    PortablePolicyError,
)
from .adapters.resources import CloudAdapterUnavailableError
from .models import (
    ApprovalDecision,
    AuditEvent,
    AuditEventType,
...
from .models.errors import StorageConflictError, StorageDependencyError
from .state.protocol import DurableTruthRepository
    ...
    def validate_completion(self) -> Self:
        if self.decision is ApprovalDecision.DENIED:
            if (
                ...
                raise ValueError(
                    "denied local completion cannot contain execution proof"
                )
        elif (
...
class BoundExecutionWorkflow:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(executor, ProtectedExecutor):
            raise TypeError("executor must implement the protected backend contract")
        if not all(
            callable(value)
    ...
    def request_approval(
        ...
        if isinstance(loaded, FailureDetail):
            return ApprovalChallengeResult.failed(loaded)
        ...
        return ApprovalChallengeResult.succeeded(
            ApprovalChallenge(
                request=request,
                proposal=proposal,
                evidence=evidence,
    ...
    def decide(
        ...
        if not isinstance(decision, DecisionRequest) or not isinstance(
            principal, CoreOperatorPrincipal
        ):
        ...
        if isinstance(loaded, FailureDetail):
            return ApprovalResolutionResult.failed(loaded)
        ...
        if existing is not None:
            ...
            if isinstance(reconciled, FailureDetail):
                return ApprovalResolutionResult.failed(reconciled)
            final_run, _ = reconciled
            return ApprovalResolutionResult.succeeded(
        ...
        if isinstance(applied, FailureDetail):
            return ApprovalResolutionResult.failed(applied)
        ...
        return ApprovalResolutionResult.succeeded(
            self._resolution(
                final_run, final_checkpoint.local_approval, reconciled=False
            )
        )
    ...
    def resume(
        ...
        loaded = self._load(run_id)
        if isinstance(loaded, FailureDetail):
            return ExecutionResult.failed(loaded)
        run, checkpoint, evidence, proposal = loaded
        ...
        if approval.decision is ApprovalDecision.DENIED:
            ...
            if isinstance(applied, FailureDetail):
                return ExecutionResult.failed(applied)
            final_run, _ = applied
        ...
        if run.state is WorkflowState.VERIFYING:
            from .verification import assert_linked_verification
            ...
            try:
                if verification is None:
                    ...
                    assert_linked_verification(receipt, verification)
                    checkpoint = self._repository.save_checkpoint(
                        self._next_checkpoint(
                            checkpoint,
                            last_safe_state=WorkflowState.APPROVED,
    ...
    def _apply_decision_state(
        ...
        try:
            ...
            if (
                not downstream_approval
                and checkpoint.last_safe_state is not target_state
            ):
                checkpoint = self._repository.save_checkpoint(
                    self._next_checkpoint(
                        checkpoint,
    ...
    def _challenge_failed(
        ...
        return ApprovalChallengeResult.failed(cls._failure(kind, code, message))
    ...
    def _resolution_failed(
        ...
        return ApprovalResolutionResult.failed(cls._failure(kind, code, message))
    ...
    def _execution_failed(
        ...
        return ExecutionResult.failed(cls._failure(kind, code, message))

    @classmethod
    def _challenge_storage_failed(cls, message: str) -> ApprovalChallengeResult:
        return ApprovalChallengeResult.failed(
            cls._failure(
                FailureKind.STORAGE_FAILURE,
                "LOCAL_STATE_STORE_UNAVAILABLE",
                message,
    ...
    @classmethod
    def _resolution_storage_failed(cls, message: str) -> ApprovalResolutionResult:
        return ApprovalResolutionResult.failed(
            cls._failure(
                FailureKind.STORAGE_FAILURE,
                "LOCAL_STATE_STORE_UNAVAILABLE",
                message,
    ...
    @classmethod
    def _execution_storage_failed(cls, message: str) -> ExecutionResult:
