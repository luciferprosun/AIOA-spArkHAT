...
# Source: agent/local_first.py at 4fafed8b1a877e55d96ddd9baea0a737fbeeaa4a.
...
from .models.digest import compute_evidence_digest
from .models.errors import StorageConflictError, StorageDependencyError
from .models.failures import workflow_state_for_failure
from .models.public_enums import ApprovalStatus, ExecutionState
...
InvestigationResult = ControlResult[InvestigationCompletion]
...
class InvestigationWorkflow:
    ...
    def __init__(
        ...
        *,
        ...
        if not all(
            callable(value) for value in (clock, proposal_id_factory, event_id_factory)
        ):
    ...
    def execute(self, run: Run, query: ResourceQuery | object) -> InvestigationResult:
            ...
            try:
                ...
                try:
                    ...
                    return self._storage_failed(
                        "Local run reconciliation is unavailable"
                    )
                if raced is None:
        ...
        if query_result.status is ResultStatus.FAILURE:
            if query_result.failure is None:
                return self._storage_failed(
                    "QueryResource returned an ambiguous failure"
                )
        ...
        # Native v1 uses a local advisor, not a billed model. Conservatively count
        # UTF-8 output bytes as token-budget units (not a tokenizer estimate).
        if isinstance(raw_plan, str):
            elapsed = max(
                0, int((self._clock() - current.created_at).total_seconds() * 1000)
            )
            units = len(raw_plan.encode("utf-8"))
            try:
                budget = current.budget.__class__(
                    **{
                        **current.budget.model_dump(),
                        "turns_used": current.budget.turns_used + 1,
                        "tokens_used": current.budget.tokens_used + units,
                        "elapsed_milliseconds_used": elapsed,
                    }
                )
            except ValueError:
                return self._persist_failure(
                    ...
                    FailureDetail(
                        ...
                        code="NATIVE_ADVISORY_BUDGET_EXHAUSTED",
                        message="Local advisory output or elapsed time exceeded the run budget",
                        retryable=False,
                    ),
                )
            try:
                current = self._repository.update_run_budget(
                    current.run_id,
                    budget,
                ...
                return self._storage_failed(
                    "Native advisory budget could not be persisted"
                )
        ...
        if plan_result.status is ResultStatus.FAILURE:
            ...
            if failure is None:
                return self._storage_failed(
                    "PlanRemediation returned an ambiguous failure"
                )
            try:
                self._append_event(
                    current,
        ...
        try:
            ...
            return self._storage_failed(
                "Protected proposal persistence or transition failed"
            )
        protected_plan = plan.model_copy(update={"proposal": awaiting_proposal})
        return InvestigationResult.succeeded(
    ...
    def _reconcile(self, run: Run) -> InvestigationResult:
        if run.state not in {
            WorkflowState.AWAITING_APPROVAL,
            WorkflowState.NO_ACTION_REQUIRED,
            WorkflowState.RECOMMENDATION_ONLY,
        ...
        if run.state is WorkflowState.NO_ACTION_REQUIRED:
            plan = RemediationPlan(
                ...
                reason=str(
                    checkpoint.resume_metadata.get("reason", "No action required")
                ),
            )
    ...
    def _persist_failure(self, run: Run, failure: FailureDetail) -> InvestigationResult:
        try:
        ...
        return InvestigationResult.failed(failure)
    ...
    @classmethod
    def _storage_failed(cls, message: str) -> InvestigationResult:
        return cls._failed(
